...
if __name__ == "__main__":
    ...
    ax1.plot(n_cluster_list, df.iloc[0:5, 1], "-o", label="[RMTF] Online K-means")
    ax1.plot(n_cluster_list, df.iloc[6:11, 1], "-o", label="[Switch+CPU] Offline K-means")
    ...
    print("[Switch+CPU] Purity: {}".format(df.iloc[6:11, 1]))
    ...
    ax3.plot(n_cluster_list, (df.iloc[0:5, 4] + df.iloc[0:5, 5]) / 2, "-o", label="[RMTF] Online K-means")
    ax3.plot(n_cluster_list, (df.iloc[6:11, 4] + df.iloc[6:11, 5]) / 2, "-o", label="[Switch+CPU] Offline K-means")
